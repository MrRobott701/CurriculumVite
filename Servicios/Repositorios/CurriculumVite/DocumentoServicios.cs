    ...
    public class DocumentoServicios : ISRepositorioDocumento
    {
        ...
        public async Task<E_Documento> CreateDocumentoForEducacionAsync(int idDocente, int idEducacion, string titulo, string url, string descripcion)
        {
            try
            {
                ...
                if (!educacionExists)
                {
                    throw new InvalidOperationException($"ERROR: La educación con ID {idEducacion} no existe en la base de datos");
                }
                Console.WriteLine($"✓ Educación con ID {idEducacion} existe");
            ...
            {
                Console.WriteLine($"Error al crear documento para educación: {ex.Message}");
                throw;
            }
        }
    }
